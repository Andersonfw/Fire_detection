"""
Created on maio 13 20:54:54 2023
...
num_superpixels = 600  # SEEDS Number of Superpixels
...
# Definindo o rótulo que representa a parte do fogo
# Converte para escala de cinza
# Define a máscara com a região de interesse
mask = np.zeros(src.shape[:2], dtype=np.uint8)
fogo_label = labels[283,510] # obtém o label do superpixel da chama do fogo
mask[labels == fogo_label] = 255

# Aplica um filtro de suavização na máscara
mask = cv2.GaussianBlur(mask, (5, 5), 0)

# Aplica a máscara na imagem original
result = cv2.bitwise_and(src, src, mask=mask)
# aplica a máscara na imagem original
L = cv2.bitwise_and(src, src, mask=mask)
print(labels)
bits = 2
# L = ((labels & (2 ** bits - 1)) * 2 ** (16 - bits)).astype(float) / ((2 ** 16) - 1)
# labels = labels.astype(float) + 1
L = label2rgb(labels, colors=['red', 'green', 'blue'], bg_label=0, bg_color=None)
...
plt.subplot(224), plt.imshow(labels[300:]), plt.title('labels')
plt.show()

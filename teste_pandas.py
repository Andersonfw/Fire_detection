...
if __name__ == "__main__":
    ...
    if os.path.exists(csvfile_training):
        ...
        for files in files_list:
                ...
                if image_name == csvimages.at[n, 'imagem']:
                    for k in range(100):
                        ...
                        if csvimages.at[n + k, 'isfire'] == 1:
                            img_df2 = subM.mount_Dataframe(listClassSubmatrix[k])
                            img_df2['Target'] = 1
                            train_df2 = pd.concat([train_df2, img_df2], ignore_index=True, axis=0)
                        else:
                            img_df2 = subM.mount_Dataframe(listClassSubmatrix[k])
                            img_df2['Target'] = 0
                            train_df2 = pd.concat([train_df2, img_df2], ignore_index=True, axis=0)
            ...
            #         if i == 36 or i == 37 or i == 45 or i == 63 or i == 72 or i == 73 or i == 82 or i == 92:
            #             img_df = subM.mount_Dataframe(listClassSubmatrix[i])
            #             img_df['Target'] = 1
            #             train_df = pd.concat([train_df, img_df], ignore_index=True, axis=0)
            #         else:
            ...
            #         if i == 37 or i == 46 or i == 56 or (i >= 61 and i <= 62) or (i >= 66 and i <= 67) or (
            #                 i >= 71 and i <= 73):
            #             img_df = subM.mount_Dataframe(listClassSubmatrix[i])
            #             img_df['Target'] = 1
            #             train_df = pd.concat([train_df, img_df], ignore_index=True, axis=0)
            ...
            #             img_df['Target'] = 0
            #             train_df = pd.concat([train_df, img_df], ignore_index=True, axis=0)
            # if train_df.equals(train_df2):
            #     print("Image igual igual", image_name)
    ...
    tn, fp, fn, tp, test_df = Ev.manualTest(knn_class, fireImageDirTesting, submatriz_height, submatriz_width, csvimages)
    ...
    #         if (i >= 2 and i <= 3) or (i >= 6 and i <= 9) or (i >= 12 and i <= 13) or (i >= 16 and i <= 19) or (
    #                 i >= 22 and i <= 23) \
    ...
    #                 or (i >= 65 and i <= 69) or (i >= 71 and i <= 73) or (i >= 75 and i <= 77) or (
    #                 i >= 81 and i <= 87) or (i >= 91 and i <= 93):
    #             img_df = subM.mount_Dataframe(listClassSubmatrix[i])
    #             img_df['Target'] = 1
    #             test_df = pd.concat([test_df, img_df], ignore_index=True, axis=0)
    #         else:
    ...
    #         if (i >= 6 and i <= 9) or i == 16 or i == 19 or (i >= 22 and i <= 23) or (i >= 26 and i <= 29) or (
    #                 i >= 32 and i <= 33) \
    #                 or (i >= 35 and i <= 39) or (i >= 42 and i <= 49) or (i >= 52 and i <= 59) or (
    #                 i >= 65 and i <= 68) or (i >= 74 and i <= 79):
    #             img_df = subM.mount_Dataframe(listClassSubmatrix[i])
    #             img_df['Target'] = 1
    #             test_df = pd.concat([test_df, img_df], ignore_index=True, axis=0)
    #         else:
